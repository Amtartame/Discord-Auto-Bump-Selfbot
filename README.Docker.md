...
### Dokploy (recommandé pour selfhosting)

[Dokploy](https://dokploy.com/) est une plateforme de déploiement self-hosted similaire à Heroku/Vercel.

**Configuration requise :**

1. **Connectez votre repo GitHub** dans Dokploy

2. **Variables d'environnement** à configurer dans Dokploy :
   ```
   USER_TOKEN=votre_token_discord
   ```

3. **Monter le fichier config.json** :

   **Option A - Créer config.json via l'interface Dokploy :**
   - Allez dans "Mounts" ou "Volumes"
   - Créez un fichier `/app/config.json` avec votre configuration :
   ```json
   {
     "bots": [
       {
   ...
   **Option B - Via volume mount :**
   - Créez un volume persistant dans Dokploy
   - Montez-le sur `/app/config.json`
   - Modifiez le fichier via SSH ou l'interface

4. **Déployez** et vérifiez les logs

**Notes importantes :**
- ⚠️ Le fichier `config.json` n'est PAS dans le repo GitHub (pour la sécurité)
- ⚠️ Vous DEVEZ le créer manuellement dans Dokploy
- ⚠️ Sans `config.json`, le bot plantera au démarrage
...
# Important : Créer config.json via Railway Dashboard
# Allez dans "Variables" > "RAW Editor" et ajoutez CONFIG_JSON avec le contenu
```
...
fly ssh console
# Puis créez /app/config.json manuellement
```
